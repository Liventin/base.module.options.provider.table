...
return $provider
    ...
    ->setShowHeader(true)  // вывести заголовок таблицы (имя опции) над таблицей
    ->getParamsToArray();
```

Заголовок над таблицей рендерится только при `setShowHeader(true)` (или `showHeader => true`
в params) и непустом `name` опции. По умолчанию заголовок не выводится — обычно имя секции
уже показывает соседний разделитель, и дублировать его не нужно.
